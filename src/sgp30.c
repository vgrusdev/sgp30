 ...
 */
static s16 sgp_i2c_read_words(int fd, u16 *data, u16 data_words) {
    s16 ret;
    u16 i, j;
    ...
    ret = user_i2c_read(fd, buf8, size);
    ...
    for (i = 0, j = 0;
        ...
        if (user_check_crc(&buf8[i], SGP_WORD_LEN,
 ...
 */
static s16 sgp_i2c_write(int fd, const sgp_command *command) {
    s8 ret;

    ret = user_i2c_write(fd, command->buf, SGP_COMMAND_LEN);
    if (ret != 0)
        return STATUS_FAIL;
 ...
 */
static s16 read_measurement(int fd, const struct sgp_profile *profile) {
    ...
    switch (client_data.current_state) {
        ...
        case MEASURING_PROFILE_STATE:
            ret = sgp_i2c_read_words(fd, client_data.word_buf,
 ...
 */
static s16 sgp_i2c_read_words_from_cmd(int fd, const sgp_command *cmd,
                                       u32 duration_us,
                                       u16 *data_words,
                                       u16 num_words) {

    if (sgp_i2c_write(fd, cmd) == STATUS_FAIL)
        return STATUS_FAIL;

    /* the chip needs some time to write the data into the RAM */
    user_sleep_usec(duration_us);
    return sgp_i2c_read_words(fd, data_words, num_words);
}
 ...
 * Return:      STATUS_OK on success, else STATUS_FAIL
 */
static s16 sgp_run_profile(int fd, const struct sgp_profile *profile) {
    u32 duration_us = profile->duration_us + 5;

    if (sgp_i2c_write(fd, &profile->command) == STATUS_FAIL)
        return STATUS_FAIL;
    ...
    if (profile->number_of_signals > 0) {
        ...
        return read_measurement(fd, profile);
    }
 ...
 * Return:      STATUS_OK on success, else STATUS_FAIL
 */
static s16 sgp_run_profile_by_number(int fd, u16 number) {
    ...
    if (sgp_run_profile(fd, profile) == STATUS_FAIL)
        return STATUS_FAIL;
...
static u16 sgp_fill_cmd_send_buf(u8 *buf, const sgp_command *cmd,
    ...
    for (i = 0; i < num_args; ++i) {
        ...
        crc = user_generate_crc((u8 *)&word, SGP_WORD_LEN);
...
s16 sgp_measure_test(int fd, u16 *test_result) {
    ...
    *test_result = 0;
    ...
    if (sgp_i2c_write(fd, &sgp_cmd_measure_test) != STATUS_OK)
        return STATUS_FAIL;

    user_sleep_usec(SGP_CMD_MEASURE_TEST_DURATION_US);

    if (sgp_i2c_read_words(fd, measure_test_word_buf,
...
s16 sgp_read_iaq(int fd, u16 *tvoc_ppb, u16 *co2_eq_ppm) {
    ...
    if (read_measurement(fd, profile) == STATUS_FAIL)
        return STATUS_FAIL;
...
s16 sgp_measure_iaq_blocking_read(int fd, u16 *tvoc_ppb, u16 *co2_eq_ppm) {
    if (sgp_run_profile_by_number(fd, PROFILE_NUMBER_IAQ_MEASURE) == STATUS_FAIL)
        return STATUS_FAIL;
...
s16 sgp_read_tvoc(int fd, u16 *tvoc_ppb) {
    u16 co2_eq_ppm;
    return sgp_read_iaq(fd, tvoc_ppb, &co2_eq_ppm);
}
...
s16 sgp_measure_tvoc_blocking_read(int fd, u16 *tvoc_ppb) {
    u16 co2_eq_ppm;
    return sgp_measure_iaq_blocking_read(fd, tvoc_ppb, &co2_eq_ppm);
}
...
s16 sgp_measure_signals_blocking_read(int fd, u16 *scaled_ethanol_signal,
    ...
    if (sgp_run_profile_by_number(fd, PROFILE_NUMBER_MEASURE_SIGNALS) == STATUS_FAIL)
        return STATUS_FAIL;
...
s16 sgp_get_iaq_baseline(int fd, u32 *baseline) {
    s16 ret = sgp_run_profile_by_number(fd, PROFILE_NUMBER_IAQ_GET_BASELINE);
    if (ret == STATUS_FAIL) {
        return STATUS_FAIL;
    }
...
s16 sgp_set_iaq_baseline(int fd, u32 baseline) {
    ...
    if (user_i2c_write(fd, buf, BUF_SIZE) != 0)
        return STATUS_FAIL;
...
s16 sgp_iaq_init(int fd) {
    return sgp_run_profile_by_number(fd, PROFILE_NUMBER_IAQ_INIT);
}
 ...
 * Return:  STATUS_OK on success.
 */
int sgp_probe(char *iic_dev) {

    int16_t  err;
    const uint64_t *serial_buf = (const uint64_t *)client_data.word_buf;
    ...
    /* Initialize I2C */
    if (( fd = user_i2c_init(iic_dev, SGP_I2C_ADDRESS)) < 0 ) {
      return fd;
    }

    /* try to read the serial ID */
    err = sgp_i2c_read_words_from_cmd(fd, &sgp_cmd_get_serial_id,
    ...
    err = sgp_i2c_read_words_from_cmd(fd, &sgp_cmd_get_featureset,
